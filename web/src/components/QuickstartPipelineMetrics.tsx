...
export function QuickstartPipelineMetrics({ metrics }: QuickstartPipelineMetricsProps) {
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center gap-2">
      <span className="text-sm font-medium leading-6 text-muted-foreground">Pipeline</span>
        ...
        return (
          <div key={step.key} className="flex items-center gap-2">
            ...
            <span className="rounded-md border border-border bg-transparent px-2 py-0.5 text-xs font-semibold leading-4 text-foreground shadow-sm">
              {step.label}
              {metric ? (
                <span className="ml-2 text-primary" title={new Date(metric.timestamp).toLocaleTimeString()}>
